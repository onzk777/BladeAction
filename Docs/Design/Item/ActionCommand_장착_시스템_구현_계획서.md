...
### 1.3 구현 방식 (2025-10-30 변경)
~~**인벤토리 시스템 통합 방식** - 검술을 ItemType.ActionCommand로 아이템화하여 기존 CharacterInventory 시스템 활용~~

**✅ 최종 구현 방식: ActionCommandData 직접 관리**
- 검술을 Item이 아닌 ActionCommandData로 직접 관리
- Character 클래스에서 습득/장착 검술 직접 관리
- ActionCommandDatabase를 통한 Key 기반 조회
- UI는 ActionCommandData를 직접 표시
...
### 2.2 단계별 진행 현황
| Phase | 작업 내용 | 실제 소요 시간 | 상태 |
|-------|-----------|----------------|------|
| Phase 1 | ~~데이터 구조 확장~~ (변경됨) | - | ❌ 스킵 |
| Phase 2 | ~~인벤토리 시스템 확장~~ (변경됨) | - | ❌ 스킵 |
| Phase 3 | Character 검술 관리 시스템 | 2시간 | ✅ 완료 |
| Phase 4 | ~~검술 아이템 생성 도구~~ (불필요) | - | ❌ 스킵 |
| Phase 5 | UI 구현 | 6시간 | ✅ 완료 |
| Phase 6 | CSV 통합 | - | ⏸️ 보류 |
| Phase 7 | 테스트 및 디버깅 | - | 🔄 진행중 |

### 2.3 설계 변경 사항
**2025-10-30**: 검술을 Item으로 관리하지 않고 ActionCommandData로 직접 관리하도록 전면 재설계
